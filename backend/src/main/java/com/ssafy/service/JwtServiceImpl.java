...
public class JwtServiceImpl implements JwtService {
    ...
    public String makeJwt(Member m) {
        ...
        expireTime.setTime(expireTime.getTime() + 1000 * 60 * 1 * 60);
        ...
        map.put("mid", m.getKey());
        map.put("email", m.getId());
        map.put("name", m.getName());
        map.put("postCount", m.getNickname());
        map.put("grade", m.getGrade());
        logger.info("jwt Token 생성: " + m.getName() + " " + m.getNickname());
	...
	public Member readJwt(String jwt) {
		Claims claims = Jwts.parser().setSigningKey(DatatypeConverter.parseBase64Binary(secretKey))
                .parseClaimsJws(jwt).getBody(); // 정상 수행된다면 해당 토큰은 정상토큰	
		Member m = new Member();
        m.setId((String) claims.get("mid"));
        m.setName((String) claims.get("name"));
        logger.info("mid :" + m.getId());
